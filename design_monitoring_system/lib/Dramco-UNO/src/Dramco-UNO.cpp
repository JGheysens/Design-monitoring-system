...
// --- Sensor readings ---
//external soil moisture & tepmerature sensor
void DramcoUnoClass::sendSoilMoisture(float soilMoisture){
    _cursor = 0;
    memset(data, '\0', DRAMCO_UNO_BUFFER_SIZE);
...
void DramcoUnoClass::addSoilMoistureToMessage(float soilMoisture){
    addSoilMoisture(soilMoisture);
}

void DramcoUnoClass::sendSoilTemperature(float soilTemperature){
    _cursor = 0;
    memset(data, '\0', DRAMCO_UNO_BUFFER_SIZE);

    addSoilTemperature(soilTemperature);
    send();
}

void DramcoUnoClass::addSoilTemperature(float soilTemperature){
    _lppAddToBuffer(soilTemperature, 0, DRAMCO_UNO_LPP_ANALOG_INPUT, DRAMCO_UNO_LPP_ANALOG_INPUT_SIZE, DRAMCO_UNO_LPP_ANALOG_INPUT_MULT);
}

void DramcoUnoClass::addSoilTemperatureToMessage(float soilTemperature){
    addSoilTemperature(soilTemperature);
}

//external rain sensor
void DramcoUnoClass::sendRain(float rain){
    _cursor = 0;
    memset(data, '\0', DRAMCO_UNO_BUFFER_SIZE);
...
void DramcoUnoClass::addRain(float rain){
    _lppAddToBuffer(rain, 0, DRAMCO_UNO_LPP_ANALOG_INPUT, DRAMCO_UNO_LPP_ANALOG_INPUT_SIZE, DRAMCO_UNO_LPP_ANALOG_INPUT_MULT);
}

void DramcoUnoClass::addRainToMessage(float rain){
    addRain(rain);
}

//external ultrasonic sensor
void DramcoUnoClass::sendUltrasonic(float ultrasonic){
    _cursor = 0;
    memset(data, '\0', DRAMCO_UNO_BUFFER_SIZE);

    addUltrasonic(ultrasonic);
    send();
}

void DramcoUnoClass::addUltrasonic(float ultrasonic){
    _lppAddToBuffer(ultrasonic, 0, DRAMCO_UNO_LPP_ANALOG_INPUT, DRAMCO_UNO_LPP_ANALOG_INPUT_SIZE, DRAMCO_UNO_LPP_ANALOG_INPUT_MULT);
}   

void DramcoUnoClass::addUltrasonicToMessage(float ultrasonic){
    addUltrasonic(ultrasonic);
}   
